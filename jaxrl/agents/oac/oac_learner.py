...
from jaxrl.agents.oac.actor import update as update_actor
...
from jaxrl.networks.common import Params
...
def _update(
    ...
    temp: Model, batch: Batch, discount: float, tau: float, target_entropy: float, beta_lb: float
    ...
    rng, key = jax.random.split(rng)
    new_actor, actor_info = update_actor(key, actor, new_critic, temp, batch, beta_lb)
...
@functools.partial(jax.jit, static_argnames=('discount', 'tau', 'target_entropy', 'num_updates', 'beta_lb'))
def _do_multiple_updates(rng: PRNGKey, actor: Model, critic: Model, target_critic: Model,
                         ...
                         target_entropy: float, beta_lb: float,
                         step, num_updates: int) -> Tuple[PRNGKey, Model, Model, Model, Model, InfoDict]:
    def one_step(i, state):
        ...
        new_rng, new_actor, new_critic, new_target_critic, new_temp, info = _update(
                ...
                jax.tree_map(lambda x: jnp.take(x, i, axis=1), batches), discount, tau, target_entropy, beta_lb)
...
class OACLearner(object):
    def __init__(self,
                 ...
                 beta_ub: float = 4.36,
                 beta_lb: float = 1.0,
                 kl_restriction: float = 3.69) -> None:
        """
        An implementation of the version of Optimistic-Actor-Critic
        """
        ...
        self.kl_restriction = kl_restriction
        self.tau = tau
    ...
    def update(self, batch: Batch, num_updates: int = 1) -> InfoDict:
        step, rng, actor, critic, target_critic, temp, info = _do_multiple_updates(
            ...
            batch, self.discount, self.tau, self.target_entropy, self.beta_lb,
...
@functools.partial(jax.jit, static_argnames=('actor_def', 'critic_def', 'beta_ub', 'kl_restriction'))
@functools.partial(jax.vmap, in_axes=(0, None, 0, None, 0, 0, None, None, None))
def _sample_actions(
        rng: PRNGKey,
        actor_def: nn.Module,
        actor_params: Params,
        ...
        temperature: float,
        beta_ub: float,
        kl_restriction: float) -> Tuple[PRNGKey, jnp.ndarray]:
        dist, mu, std = actor_def.apply({'params': actor_params}, observations, temperature=temperature, return_params=True)
        def get_loss(mu):
            tanh_mu = nn.tanh(mu)
            q1, q2 = critic_def.apply({'params': critic_params}, observations, tanh_mu)
            q_mu = (q1 + q2) / 2.0
            q_std = jnp.abs(q1 - q2) / 2
            q_ub = q_mu + beta_ub * q_std
            return q_ub
        grad_fn = jax.grad(get_loss)
        grad_critic = grad_fn(mu)
        mu_delta = kl_restriction * (grad_critic * std ** 2) / (jnp.sqrt((grad_critic ** 2 * std ** 2).sum()) + 1e-6)
        mu_explore = mu + mu_delta
        oac_dist = tfd.MultivariateNormalDiag(loc=mu_explore,
                                               scale_diag=std)
        rng, key = jax.random.split(rng)
        actions = oac_dist.sample(seed=key)
...
def sample_actions(
        ...
        optimism: float = 2.0,
        kl_restriction: float = 3.69) -> Tuple[PRNGKey, jnp.ndarray]:
    return _sample_actions(rng, actor_def, actor_params, critic_def, critic_params, observations,
                           temperature, optimism, kl_restriction)
