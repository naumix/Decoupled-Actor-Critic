...
@functools.partial(jax.vmap, in_axes=(0, 0, 0, 0, 0, 0, None, None, None, 0))
def _update(
    rng: PRNGKey, actor: Model, target_actor: Model, critic: Model, target_critic: Model, batch: Batch,
    discount: float, tau: float, step: int, optimism: jnp.ndarray
) -> Tuple[PRNGKey, Model, Model, Model, Model, InfoDict]:
    rng, key = jax.random.split(rng)
    new_critic, critic_info = update_critic(key, target_actor, critic, target_critic,
                                            batch, discount, optimism)
    
    def pos_f():
        new_actor, _ = update_actor(actor, new_critic, batch, optimism)
        return new_actor
    ...
    _, actor_info = update_actor(actor, new_critic, batch, optimism)
    new_actor = jax.lax.cond((step%2==0), pos_f, neg_f)
    
    new_target_critic = target_update(new_critic, target_critic, tau)
    new_target_actor = target_update(new_actor, target_actor, tau)

    return rng, new_actor, new_target_actor, new_critic, new_target_critic, {
        **critic_info,
        **actor_info,
    }
...
def _do_multiple_updates(rng: PRNGKey, actor: Model, target_actor: Model, critic: Model, target_critic: Model,
                         batches: Batch, discount: float, tau: float, optimism: jnp.ndarray,
                         num_updates: int, step: int) -> Tuple[PRNGKey, Model, Model, Model, Model, InfoDict]:
    
    def one_step(i, state):
        step, rng, actor, target_actor, critic, target_critic, info = state
        step = step + 1
        new_rng, new_actor, new_target_actor, new_critic, new_target_critic, info = _update(
                rng, actor, target_actor, critic, target_critic,
                jax.tree_map(lambda x: jnp.take(x, i, axis=1), batches), discount, tau, step, optimism)
        return step, new_rng, new_actor, new_target_actor, new_critic, new_target_critic, info
    
    step, rng, actor, target_actor, critic, target_critic, info = one_step(0, (step, rng, actor, target_actor, critic, target_critic, {}))
    return jax.lax.fori_loop(1, num_updates, one_step,
                            (step, rng, actor, target_actor, critic, target_critic, info))
...
class DDPGLearner(object):
    ...
    def update(self, batch: Batch, num_updates: int, optimism: jnp.ndarray) -> InfoDict:
        self.step, rng, self.actor, self.target_actor, self.critic, self.target_critic, info = _do_multiple_updates(self.rng, self.actor, self.target_actor,
                                                                                                    self.critic, self.target_critic,
                                                                                                    batch, self.discount,
                                                                                                    self.tau, optimism,
                                                                                                    num_updates, self.step)
        self.rng = rng
        return info
